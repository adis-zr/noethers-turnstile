...
The experiments follow the theorem architecture. We first hold evidence fixed and refine permission scales. Across communication, inference and aviation-geometry examples, densification distinguishes three regularity classes: smooth quantization artifacts, genuine structural thresholds and evidence-axis transitions. We then hold the permission scale fixed and alter evidence representation. Evidence hiding produces conservative permission descent. Projection fidelity exposes the admissibility boundary: a compact representation can look natural while erasing the permission skeleton and spuriously restoring authorization. Finally, we move both axes simultaneously. In the central experiment, 21 cases spanning the entire implemented permission chain are evaluated across resolving and non-resolving evidence paths and permission-grid refinements. The resolving path is sound and monotone in every cell and converges for every case. The non-resolving path loses soundness and evidence-axis monotonicity exactly where it passes through a skeleton-truncating projection, then reconverges once it enters the resolving tail.
...
| Implemented evidence projection is sound only when authorization-admissible | Theorem and projection-fidelity test |
| Resolving evidence refinement recovers \(A(e)\) semantically | Theorem |
| Joint evidence and permission refinement converges to \(A(e)\) from below | Theorem and full-chain implemented demonstration |
| Active evidence refinements can shift permission when they resolve active blockers | Synthetic control demonstration |
| Later refinements can be inert when no remaining split crosses a permission boundary | Analytic consequence and observed behavior |
| Skeleton-truncating projection can spuriously restore authorization | Implemented non-vacuity witness |
| Non-resolving paths may reconverge but lose monotone guarantees | Implemented boundary test |
...
| Two-axis convergence matrix | Central implemented conservation result | Resolving evidence and permission refinement recover \(A(e)\) across the full permission chain |
| Active-refinement witnesses | Control for later refinements | Later evidence splits can carry signal when they resolve active blockers |
| Non-resolving skeleton-truncating path | Boundary test for theorem hypotheses | A non-admissible projection can lose soundness and monotonicity before reconverging in a resolving tail |
...
The previous experiments exercise the two approximation axes separately. Permission densification fixes evidence and refines the grid. Occlusion and projection fidelity fix the grid and alter evidence representation. The central experiment moves both axes at once.

The experiment evaluates two paths. The resolving path uses an admissible join projection at every evidence level. The non-resolving path prepends a skeleton-truncating two-gap projection at L5 and then enters the same admissible resolving tail from L4 onward. Both paths are evaluated across the permission-grid values

$$
k\in\{4,8,16,32,64\}.
$$

The implemented medical-AI hierarchy is a finite chain whose named permission levels are present in every tested grid. The permission-axis checks therefore test that embedding the categorical hierarchy into finer grids introduces no reversals; the quantitative behavior of permission densification itself is tested separately in Section 2.4. The joint experiment asks a different question: whether evidence refinement and permission-grid refinement preserve soundness and converge to the same latent authorization value across a full set of cases.

#### 2.7.1 Case coverage spans the full permission chain

The expanded run contains 21 cases:

- 7 induction cases: M01--M07;
- 5 held-out cases: H01--H05;
- 5 synthetic level-ladder cases: S-REF, S-DIA, S-REV, S-AEX and S-ALR;
- 4 active-refinement witnesses: W-currency, W-deployment, W-population and W-clinical.

For every case, the reference authorization \(A(e)\) is computed under the admissible join projection. The case set spans the full implemented permission chain:

$$
\mathrm{REF}\prec\mathrm{DIA}\prec\mathrm{REV}\prec\mathrm{AEX}\prec\mathrm{ALR}.
$$

This coverage matters. The earlier version of the experiment recovered a middle permission value in two Epic cases. The v3 run tests the conservation behavior across all permission levels, including synthetic ladder controls and active-refinement witnesses.

#### 2.7.2 Resolving path
...
where L4 is a coarse but admissible five-gap representation and L0 is the full nine-gap taxonomy. The path progressively splits evidence currency, deployment control, population scope and model adequacy composites back into their fine obligations.

The resolving path contains 525 tested cells: 21 cases, 5 evidence levels and 5 permission-grid values. Every cell is sound. Every evidence-axis transition is monotone. Every permission-axis transition is monotone. Every case converges to its reference authorization value.
...
This is the positive conservation result. Along an admissible resolving path, the compiler recovers \(A(e)\) from below across the full permission chain.

#### 2.7.3 Active-refinement witnesses show that later splits can carry signal

In the Epic induction cases, later refinements after the first active split are often inert. That behavior is correct if no remaining split crosses a permission boundary, but it raises a natural concern: perhaps later refinements are inert because the implementation cannot express signal at those levels.

We therefore add a parallel family of active-refinement witnesses using the same conservative composite semantics. Each witness targets one split. The fine ALR profile installs one closed requirement on one component paired with a bounded requirement on its sibling. The projected composite status is the meet of component statuses, so the bounded sibling poisons the composite. The projected composite ALR requirement is the join of the fine requirements, so the composite must be closed. ALR is blocked at the merged level. After the split, the strict component is visible, satisfies its closed requirement, and the bounded sibling no longer poisons it.

Each witness fires exactly at its target split and is inert elsewhere.
...
This control resolves the inertness concern. Later refinements are not inert by construction. They move permission exactly when they resolve an active blocker. Epic tail inertness therefore means that no remaining active blocker is present, not that the refinement machinery has run out of signal.

#### 2.7.4 Non-resolving path exposes the theorem boundary
...
The non-resolving path contains 630 tested cells: 21 cases, 6 evidence levels and 5 permission-grid values. It remains monotone in permission refinement in all cells, and it still converges at the final resolving endpoint for every case. But it loses the guarantees that require admissibility and resolving refinement.

There are 10 unsound cells. All occur at L5. They are H02 and S-REV across all five grid values. In both cases the reference authorization is REV, while the skeleton-truncating L5 projection emits AEX.

There are 80 evidence-axis monotonicity failures. All occur at the transition from L5 to L4, where the admissible permission skeleton is restored and inflated permissions drop. These 80 failures span 16 of the 21 cases. The five cases that do not fail at this transition are cases where the skeleton-truncating projection could not inflate the emitted permission in the first place.

| Path | Cells | Sound | Monotone in evidence refinement | Monotone in permission refinement | Jointly converged |
|---|---:|---:|---:|---:|---:|
| Non-resolving | 630 | 620 | 550 | 630 | 21/21 |

The failure decomposition is important.

| Failure type | Count | Location | Interpretation |
|---|---:|---|---|
| Unsound cells | 10 | L5, H02 and S-REV across all \(k\) | Skeleton-truncating projection emits AEX above \(A(e)=\mathrm{REV}\) |
| Evidence-axis monotonicity failures | 80 | L5→L4 across 16 cases | Restoring the admissible skeleton lowers inflated permission |
| Permission-axis monotonicity failures | 0 | none | Permission-grid refinement remains well behaved |
| Final convergence failures | 0 | none | Resolving tail recovers \(A(e)\) |

The non-resolving path does not contradict the theorem. The theorem gives sufficient conditions for sound, monotone convergence along the way. A path may pass through a non-admissible projection and still reconverge after entering a resolving tail. What it loses is the guarantee before that tail. The experiment shows exactly this: soundness and evidence-axis monotonicity fail at the skeleton-truncating step, and convergence resumes once the admissible skeleton is restored.

#### 2.7.5 Central result

Across all 21 cases and all permission levels, resolving refinement recovers the latent authorization value from below with no soundness or monotonicity violations. Active-refinement witnesses show that later evidence splits shift permission when they resolve active blockers. A non-resolving skeleton-truncating path loses soundness and evidence-axis monotonicity exactly at the non-resolving projection, then reconverges once it enters the resolving tail.

This is the implemented conservation law.

![Figure 5. Two-axis convergence across 21 cases.](figures/fig5_two_axis_convergence.png)

![Figure 6. Active-refinement witnesses and non-resolving boundary behavior.](figures/fig6_active_and_nonresolving_controls.png)
...
Third, joint refinement can be tested. The full-chain experiment demonstrates that a resolving evidence path and permission-grid refinement recover the latent authorization value from below across REF, DIA, REV, AEX and ALR. The active-refinement controls show that later evidence splits can shift permission when they resolve active blockers. The non-resolving path shows that a skeleton-truncating projection can lose soundness and evidence-axis monotonicity exactly where the theorem's hypotheses fail, while still reconverging after entering a resolving tail.
...
### 4.8 Two-axis convergence matrix

The v3 two-axis experiment is implemented in:
...
where `path` is resolving or non-resolving, `case` is one of the 21 cases, \(m\) is the evidence representation level, and \(k\) is the permission-grid value. The row records categorical permission, normalized compiler score \(C_{m,k}\), reference authorization \(A(e)\), gap, soundness, evidence-axis monotonicity and permission-axis monotonicity.
...
The active-refinement witnesses are synthetic controls under the same conservative composite semantics used in the admissible projection path.
...
This lemma explains the active-refinement witnesses. It also explains why a resolving tail can be inert in ordinary cases: if no composite contains such a poisoned active blocker, splitting it reveals no new permission-satisfying structure. \(\square\)

---

## A.10 Non-resolving path boundary

Theorem A.3 gives sufficient conditions for monotone convergence from below: resolving evidence refinement, permission-grid refinement and asymptotic meet-exactness of the implementation. These conditions are not necessary for eventual endpoint recovery.

A path may pass through a non-admissible projection \(\pi_0\) with implemented authorization \(\widehat A^{\pi_0}\) satisfying

$$
\widehat A^{\pi_0}(\pi_0(e))\npreceq A(e)
$$

for some evidence states \(e\), then later enter an admissible resolving tail. On the non-admissible prefix, soundness and monotone evidence-refinement guarantees need not hold. Once the path enters a resolving asymptotically meet-exact tail, the tail may still recover \(A(e)\) from below.

This is the behavior observed in the non-resolving path. L5 is skeleton-truncating and can emit AEX above \(A(e)=\mathrm{REV}\). Restoring the admissible skeleton at L4 lowers inflated permissions, causing evidence-axis monotonicity failures. The path nevertheless reaches the same L0 endpoint as the resolving path. The failure is not final convergence; it is the loss of soundness and monotone guarantee before the resolving tail. \(\square\)
...
The full matrix contains one row per path, case, evidence level and permission-grid value. Fields are:
...
The non-resolving path has 10 unsound cells and 80 evidence-axis monotonicity failures. All unsound cells occur at L5. All evidence-axis monotonicity failures occur at L5→L4.

## Supplementary Note 6. Active-refinement controls

The active-refinement controls instantiate Appendix A.9. Each witness targets one split and uses the same conservative composite semantics.
...
Each transition is strict at the target split and inert elsewhere. This confirms that the resolving path can carry signal at every refinement level when an active blocker is present.
...
**Figure 5. Two-axis convergence.** Resolving path: across 21 cases, an admissible resolving evidence path and permission-grid refinement recover \(A(e)\) from below with 525/525 sound cells, 525/525 evidence-axis monotonicity checks, 525/525 permission-axis monotonicity checks and 21/21 final convergence. Non-resolving path: a skeleton-truncating L5 projection introduces unsoundness and monotonicity failures before the path reconverges in the resolving tail.

**Figure 6. Active-refinement witnesses and theorem boundary.** The four active-refinement witnesses shift permission exactly at their target splits and remain inert elsewhere. The non-resolving path loses soundness only at L5 and evidence-axis monotonicity only at L5→L4, showing that the theorem hypotheses fail at a specific representation boundary rather than diffusely across the path.
