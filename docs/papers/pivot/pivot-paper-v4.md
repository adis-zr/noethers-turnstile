...
At every level it uses the admissible join projection: composite statuses are meets of component statuses, and composite requirements are joins of inherited fine requirements. The permission-grid values are
...
The failure decomposition is more informative than the aggregate counts. There are 10 unsound cells. All occur at L5. They are H02 and S-REV across all five grid values. In both cases the reference authorization is REV, while the skeleton-truncating L5 projection emits AEX.

There are 80 evidence-axis monotonicity failures in the full stress test, all at the transition L5→L4. These failures occur when the admissible skeleton is restored and inflated permissions drop. In the canonical submatrix, 60 such failures occur: 12 canonical cases across five grid values. The remaining 20 occur in the four active-refinement controls. The five cases with no L5→L4 monotonicity failure are H03, S-REF, S-DIA, S-AEX and S-ALR; in those cases the truncating projection cannot produce a higher emit than the restored admissible projection at that transition.

This pattern is structural. Let \(K\) be the set of permission-grid values, with \(|K|=5\). Let
...
$$
r(X)=r(x_s)ee r(x_p)=\mathrm{closed}.
$$
...
and the restoration-drop set has size \(|R|=16\). With five grid values, each truncating step contributes \(2\cdot5=10\) unsound cells and each restoration transition contributes \(16\cdot5=80\) evidence-axis monotonicity failures. A terminal truncation has no restoration transition and lacks a resolving tail; it converges in 19 of 21 cases.
...
Let \((\pi_m,\widehat A_m)\) be an implemented evidence-projection path, and suppose one step \(j\) uses a skeleton-truncating projection \(T\) followed by a restored admissible projection \(R\) at step \(j+1\). Let \(K\) be the set of permission grids tested. Define the unsound-eligible set

$$
U_T=\{e:\widehat A_T(\pi_T(e))\succ A(e)\},
$$

and the restoration-drop set

$$
D_{T,R}=\{e:\widehat A_T(\pi_T(e))\succ \widehat A_R(\pi_R(e))\}.
$$

For each grid \(k\in K\), the finite compiler output is the floor of the implemented authorization at that grid. When the truncating and restored outputs lie on grid-resolved permission levels, the number of unsound cells contributed by the truncating step is

$$
|U_T|\,|K|,
$$

and the number of evidence-axis monotonicity failures contributed by the restoration transition is

$$
|D_{T,R}|\,|K|.
$$

The proof is by direct expansion of the cell definitions. A cell at the truncating step is unsound exactly when the truncating implemented permission exceeds \(A(e)\). This condition is independent of the grid value in the reported finite-chain experiment because the relevant permissions are grid-resolved categorical levels. Thus each evidence state in \(U_T\) contributes one unsound cell for each \(k\in K\), and no other evidence state contributes one. Similarly, a restoration transition violates monotonicity exactly when the emitted permission drops from the truncating representation to the restored representation. Each evidence state in \(D_{T,R}\) contributes one such violation for each grid value, and no other evidence state contributes one. \(\square\)

In the path-shape experiment, \(|K|=5\), \(U_T=\{\mathrm{H02},\mathrm{S	ext{-}REV}\}\), and \(|D_{T,R}|=16\). Therefore each truncating step contributes \(2\cdot5=10\) unsound cells, and each restoration transition contributes \(16\cdot5=80\) evidence-axis monotonicity failures.
...
$$
igvee_{M\geq t}\ igwedge_{m\geq M}\widehat A_m(\pi_m(e))
$$
...
The full two-axis matrix contains one row per path, case, evidence level and permission-grid value. Fields are:
