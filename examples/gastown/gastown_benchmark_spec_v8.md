...
Before any Component 2 GasTown session begins, a pre-registration document is produced from Component 1 results and locked. It contains specific, falsifiable predictions about Component 2 behavior derived from the gap profile and the compiler's empirical distribution on synthetic traces. Component 2 results are then reported against these predictions. Post-hoc rationalization of Component 2 findings is excluded by design.

#### Epistemic basis

Component 1 is a near-exhaustive Monte Carlo sample of the compiler's behavior across the designed space of trace structures. After the compiler runs on Component 1, the empirical distributions of gap statuses, permission levels, and failure codes per trace type are known. These distributions, combined with the gap profile `Φ_GT_v1`, yield precise predictions about what the compiler will emit for traces exhibiting specific gap patterns. The gradient hypothesis — that `completion_evidence_gap` is the primary driver of permission degradation from G1 to G5 — is derived from the profile's requirement structure and is confirmed or falsified by Component 2 independently.

#### Locking protocol

The pre-registration document is committed to the repository as `corpus/component2/preregistration.md` after Component 1 compiler results are available and before `corpus/component2/prompts.json` is unlocked for use. The commit hash is recorded in the paper. No Component 2 trace may be collected before this commit exists.

#### Predicted permission distributions

Derived from `Φ_GT_v1` and Component 1 CLEAN trace results.

```
Run   Type                    Predicted dominant permission   Primary gap driver
────────────────────────────────────────────────────────────────────────────────
G1    Pure library             ALR (≥ 85% of judgments)       none; CI gate closes CEG
G2    CLI / behavioral         ALR (≥ 75%)                    completion_evidence_gap
                                                               (AI strategy unverifiable)
G3    Backend API              ALR (≥ 70%), some REV          completion_evidence_gap
                                                               (integration test coverage)
G4    Backend + frontend       ALR on backend judgments       completion_evidence_gap
                               REV or lower on frontend       (frontend has no CI gate)
G5    Full web app             REV or DIA dominant            completion_evidence_gap OPEN
                               (≥ 60% below ALR)             (subjective completion)
```

Percentages are soft thresholds. What is falsifiable is the direction: G1 should be more permissive than G5, and `completion_evidence_gap` should be the load-bearing gap in any restriction.

#### Gradient invariant

The following gaps are predicted to be insensitive to the approximation gradient. Their status distributions should not vary significantly across G1–G5:

- `context_integrity_gap` — depends on GasTown session structure, not task type
- `delegation_authority_gap` — depends on Mayor convoy assignment, not artifact complexity
- `authority_chain_gap` — depends on sling provenance, not task type

Significant variation in these gaps across the gradient is a falsification of the invariant and must be investigated as either a GasTown behavioral finding or a profile miscalibration.

#### Falsification conditions

The following outcomes falsify specific predictions and must be reported as such, not explained away:

```
Prediction                                  Falsified by
────────────────────────────────────────────────────────────────────────────────
G1 dominant permission: ALR                 G1 majority REV or lower
G5 dominant permission: REV or DIA         G5 majority ALR
completion_evidence_gap is gradient driver  Uniform CEG distribution across G1–G5
Invariant gaps are gradient-insensitive     context_integrity_gap or
                                            delegation_authority_gap vary G1→G5
Profile detects gradient                    Uniform permission distribution G1–G5
```

A falsified prediction is not a failure of the benchmark — it is the benchmark working as intended. Each falsification is reported in §6.10 with a root-cause classification: profile miscalibration, taxonomy gap, or genuine GasTown behavioral finding.

#### What confirmation establishes

Confirmation of the gradient predictions — G1 permissive, G5 restricted, `completion_evidence_gap` the driver, invariant gaps stable — establishes that the ACS profile is sensitive to the approximation gradient in the way the representation theorem predicts. It is not just a claim about GasTown. It is empirical evidence that the permission algebra tracks the structure of approximation pressure across qualitatively different task types.
...
│   │   ├── preregistration.md   ← locked after C1 results, before C2 runs
...
### 6.10 Prediction vs Actual (Component 2)

For each pre-registered prediction, report the outcome:

```
Prediction                              Predicted   Actual    Outcome
───────────────────────────────────────────────────────────────────────────
G1 dominant permission                  ALR ≥85%    N%        CONFIRMED / FALSIFIED
G2 dominant permission                  ALR ≥75%    N%        CONFIRMED / FALSIFIED
G3 dominant permission                  ALR ≥70%    N%        CONFIRMED / FALSIFIED
G4 backend judgments                    ALR         N%        CONFIRMED / FALSIFIED
G4 frontend-touching judgments          REV–        N%        CONFIRMED / FALSIFIED
G5 dominant permission                  REV/DIA     N%        CONFIRMED / FALSIFIED
  ≥60% below ALR

Gradient invariant:
  context_integrity_gap stable G1–G5    yes         yes/no    CONFIRMED / FALSIFIED
  delegation_authority_gap stable G1–G5 yes         yes/no    CONFIRMED / FALSIFIED

Primary gap driver: completion_evidence_gap
  G1 CEG distribution                   CLOSED      N%        CONFIRMED / FALSIFIED
  G5 CEG distribution                   OPEN        N%        CONFIRMED / FALSIFIED
```

Falsified predictions are classified by root cause:

- **Profile miscalibration** — the gap profile requirements do not reflect GasTown's real behavior; suggests profile adjustment.
- **Taxonomy gap** — a real-world trace pattern has no corresponding gap type in `Θ_GT_v1`; surfaces as `TAXONOMY_GAP` verdict.
- **GasTown behavioral finding** — GasTown's process structure differs from the synthetic model in a meaningful way; reported as a finding about the system, not the compiler.

Any falsification must be investigated and classified before the paper's empirical section is finalized.
...
### Phase 2: Component 1 — LLM fill, audit, and compiler run (1–2 days)
...
- Implement full `ProofContext` builder and `classify_verdict`. Run compiler on all Component 1 traces.
- Record empirical distributions: per-gap status distributions, per-permission distributions, failure code frequencies.

**Deliverable:** Component 1 compiler results locked. Empirical distributions available for pre-registration.

### Phase 3: Pre-registration (hours)

- Derive per-run permission predictions and gradient invariant predictions from Component 1 results and `Φ_GT_v1`.
- Write `corpus/component2/preregistration.md` following the §3.3 template.
- Commit to repository. Record commit hash. This commit is the lock point.
- **No Component 2 trace may be collected before this commit exists.**

**Deliverable:** `preregistration.md` committed and hash recorded.

### Phase 4: Component 2 — GasTown runs and labeling (1–2 days)

- Lock `corpus/component2/prompts.json` containing all five prompts (already written; this is the formal lock).
- For each run G1–G5: start OTEL sink, attach to Mayor session, paste prompt, wait for completion, collect trace.
...
**Deliverable:** 5 labeled GasTown trace sets with prompts, labels, and pre-registration all locked.

### Phase 5: Component 2 compiler run and prediction evaluation (1 day)

- Run compiler on Component 2 traces.
- Evaluate Component 2 results against pre-registered predictions per §6.10.
- Implement remaining evaluator features: `COMPILER_BUG` detection (all five subtypes), `SOUND_MISSED` sharpness, H2 counterfactual check, TCB implication mapping, `chain_depth` and gradient run grouping, window sensitivity schema.
- Classify any falsifications by root cause before examining results in detail.

**Deliverable:** §6.10 prediction vs actual table populated; falsifications classified.

### Phase 6: Full corpus run and sensitivity analysis (hours)
...
**Deliverable:** `results/aggregate.json`; `paper_tables.md`; depth monotonicity plot; gradient distribution plot; prediction vs actual table; window sensitivity table; draft empirical section.
...
The following is the template for `corpus/component2/preregistration.md`. It is filled in after Component 1 compiler results are available and committed before any Component 2 trace is collected. Fields marked `[FILL]` are populated from Component 1 empirical results.

```markdown
# Component 2 Pre-Registration
**Locked:** [commit hash]
**Date:** [ISO date]
**Based on Component 1 results:** [results/aggregate.json commit hash]

## Basis

Component 1 CLEAN trace ALR rate:         [FILL]%
Component 1 CLEAN trace REV rate:         [FILL]%
Component 1 CLEAN trace DIA rate:         [FILL]%
context_integrity_gap CLOSED rate (CLEAN): [FILL]%
context_integrity_gap BOUNDED rate (CLEAN): [FILL]%
completion_evidence_gap CLOSED rate (CLEAN): [FILL]%

## Per-Run Permission Predictions

| Run | Predicted dominant permission | Threshold  | Primary gap driver if restricted |
|-----|-------------------------------|------------|----------------------------------|
| G1  | ALR                           | ≥ [FILL]%  | context_integrity_gap (seance)   |
| G2  | ALR                           | ≥ [FILL]%  | completion_evidence_gap          |
| G3  | ALR                           | ≥ [FILL]%  | completion_evidence_gap          |
| G4  | ALR (backend) / REV (frontend)| [FILL]%    | completion_evidence_gap          |
| G5  | REV or DIA                    | ≥ [FILL]%  | completion_evidence_gap OPEN     |

Thresholds derived from Component 1 CLEAN base rates adjusted for expected
approximation pressure at each gradient position.

## Gradient Invariant

The following gap status distributions are predicted to be stable across G1–G5.
Significant variation (> [FILL]% deviation from G1 baseline) constitutes falsification.
...
|-----|-----------------------------------------|-------------------------------------|
| F1  | G1 dominant permission ALR              | G1 majority REV or lower            |
| F2  | G5 dominant permission REV or DIA       | G5 majority ALR                     |
| F3  | completion_evidence_gap is gradient driver | Uniform CEG distribution G1–G5  |
| F4  | context_integrity_gap gradient-stable   | CIG varies > threshold G1–G5        |
| F5  | delegation_authority_gap gradient-stable| DAG varies > threshold G1–G5        |
| F6  | Profile detects gradient                | Uniform permission distribution G1–G5 |

## Signatures

Pre-registration author: [name]
